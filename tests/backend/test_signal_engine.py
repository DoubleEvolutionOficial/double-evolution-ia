...
from app.laboratory import AnalysisEvent, LaboratoryEngine, SignalEngine


def test_signal_engine_handles_empty_history():
    report = SignalEngine(LaboratoryEngine()).build_signal_report()
...
def test_signal_engine_combines_laboratory_components():
    ...
    report = signal_engine.build_signal_report()

    assert report["signal"] in {"ENTER", "WAIT", "AVOID"}
    assert 0.0 <= report["signal_strength"] <= 100.0
    assert 0.0 <= report["probability"] <= 100.0
    assert 0.0 <= report["confidence"] <= 100.0
    assert 0.0 <= report["risk"] <= 100.0
    assert report["recommendation"]
    assert report["explanation"]